...
namespace Business.Files
{
    public class OverlayIconInfo
    {
        ...
        public OverlayIconInfo(FileInfo fi)
        {
            ExtractIcon(fi.FullName, false);
        }
        public OverlayIconInfo(DirectoryInfo di)
        {
            ExtractIcon(di.FullName, true);
        }
        public OverlayIconInfo(string path, bool isFolder)
        {
        ...
        private void GetOverlayIconGuid()
        {
            ...
            foreach (string key in ShellIconOverlayIdentifiers.Keys)
            {
                ...
                var shIconOverlayIndex = SHGetIconOverlayIndex(AIconFileName, AIconIndex);
                if (shIconOverlayIndex == IconOverlayIndex)
                {
